...
The dataset utilized in this report is called "Psychosocial Dimensions of Student Life", available on: https://www.kaggle.com/datasets/mdismielhossenabir/psychosocial-dimensions-of-student-life. The dataset has 10 columns (variables) and 78 observations. These columns and rows describe characteristics of computer science students and aims to identify the depression level in these students.
...
Going ahead, this new table detail a litlle more the information about categorical (factor) variables. Here we can see all their levels and the frequency and percentage of that levels occurrences. 

As said before, our primary objective is to discover information about the variable DepressionStatus. To do so, let's investigate it a little closer.

DepressionStatus has 3 levels, and 2 of them inform that the students have some symptoms of depression (Yes and Sometimes). The most ansewered levels was Sometimes (~ 44%) and Yes(~ 34%), consecutively.

These results demonstrate that most of the students (79%) have some sympthons of depression.

```{r echo=FALSE}
dlookr::plot_bar_category(data_student, DepressionStatus)
```

This bar plot demonstrate the difference of the frequency in the levels graphically. obs: the blue line indicates de mean.
...
The Chi-squared test for given probabilities / goodness of fit of challenge_depression to a uniform
